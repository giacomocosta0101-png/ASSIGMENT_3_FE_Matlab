...
% LHP price
price_LHP_value = tranchePriceLHP_Vasicek(p, rho, LGD, Kd, Ku, disc_fact, notional);
price_LHP = price_LHP_value * ones(size(I_grid));
...
% Compute exact prices for all feasible I
    price_exact(idx_exact) = arrayfun(@(I) ...
       tranchePriceExact_Vasicek(I, p, rho, LGD, Kd, Ku, disc_fact, notional), I_small);
end

% KL approximation
price_KL = arrayfun(@(I) ...
    tranchePriceKL_Vasicek(I, p, rho, LGD, Kd, Ku, disc_fact, notional), I_grid);
end